...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && post('acao') === 'importar') {
    ...
    try {
        $resultado = restaurarBackup($up['tmp_name']);
        flash('Backup importado' . ($resultado['migracoes'] ? ', com ' . $resultado['migracoes'] . ' migração(ões) aplicada(s)' : '')
            . '. O estado anterior ficou guardado em backups/' . basename($resultado['seguranca']) . '.');
    } catch (Throwable $e) {
        flash('Restauração não concluída: ' . $e->getMessage(), 'erro');
    }
    redirect('backup.php');
}
